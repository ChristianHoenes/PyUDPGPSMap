        ...
        while not self.quit:
            ...
            try:
                ...
                while not self.quit:
                    ...
                    message = struct.unpack("<llLLLLLHcc",data)
                    checksum = checksum + sum(map(int,str(message[0])))
                    checksum = checksum + sum(map(int,str(message[1])))
                    checksum = checksum + sum(map(int,str(message[2])))
                    checksum = checksum + sum(map(int,str(message[3])))
                    checksum = checksum + sum(map(int,str(message[4])))
                    checksum = checksum + sum(map(int,str(message[5])))
                    checksum = checksum + sum(map(int,str(message[6])))
                    if message[3] > time:
                        print(checksum)
                        time = message[3]
                        print(message)
